...
class Books(db.Model):
    ...
    def __init__(self, bookName, author, yearPublished, typeLoan, book_status):
...
class Loans(db.Model):
    ...
    loan_activate = db.Column(db.String(50))

    def __init__(self, CustomerID, BookID, Loandate, Returndate, loan_status, loan_activate):
...
def books(id=-1):
    if request.method == 'POST':
        ...
        newBook = Books(bookName, author, yearPublished, typeLoan, book_status)
    ...
    if request.method == 'PUT':
           request_data = request.get_json()
           UpdBook = Books.query.get(id)
           if UpdBook:
                UpdBook.book_status = request_data["book_status"]
                db.session.commit()
...
def loans(id=-1):
    if request.method == 'POST':
        ...
        newLoan = Loans(CustomerID, BookID, Loandate,
                        Returndate, loan_status, loan_activate)
